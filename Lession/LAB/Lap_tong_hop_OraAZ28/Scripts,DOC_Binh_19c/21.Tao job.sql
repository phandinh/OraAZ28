...
CREATE TABLE oaz22.dbamf_log_op
    (id                             NUMBER,
...
CREATE SEQUENCE oaz22.dbamf_log_op_SEQ
  START WITH 1
...
CREATE TABLE oaz22.DBAMF_LOG_JOBS
(
  ID          NUMBER,
...
--DROP SEQUENCE oaz22.DBAMF_LOG_JOBS_SEQ; 

CREATE SEQUENCE oaz22.DBAMF_LOG_JOBS_SEQ
  START WITH 1
  MAXVALUE 9999999999999999999999999999
  MINVALUE 1
  NOCYCLE
...
select * from oaz22.DBAMF_LOG_OP;

select oaz22.DBAMF_LOG_OP_SEQ.nextval from dual;
...
create   or replace PROCEDURE sys.auto_extend_space
    IS
       ...
       free_space_low_level   NUMBER := 50000; 
        ...
        and round(maxbytes/1048576-(ROUND (a.bytes_alloc / 1024 / 1024)- ROUND (NVL (b.bytes_free, 0) / 1024 / 1024)),0) < 100000
    ...
    BEGIN
        insert into oaz22.dbamf_log_jobs (id,name,status,event_date, note)
            values(oaz22.dbamf_log_jobs_seq.nextval,'Starting sys.dba_op.extend_space',1,sysdate,'oaz22.dbamf_log_jobs, oaz22.dbamf_log_op');
        commit;
        ...
        LOOP
            BEGIN
                ...
                insert into oaz22.dbamf_log_op (name, type, note) values (v_sql, 'df','Add a new datafile');
                commit;
            EXCEPTION
                WHEN OTHERS
                THEN
                   v_err := substr(SQLERRM,1,200);        
                    insert into oaz22.dbamf_log_jobs (id,name,status,event_date, note)
                        values(oaz22.dbamf_log_jobs_seq.nextval,'Error sys.dba_op.extend_space',-1,sysdate,'Error sys.dba_op.extend_spac, '||v_err);
                commit;
            END;
        END LOOP;

        insert into oaz22.dbamf_log_jobs (id,name,status,event_date, note)
            values(oaz22.dbamf_log_jobs_seq.nextval,'Completed sys.dba_op.extend_space',1,sysdate,'oaz22.dbamf_log_jobs');
        commit;
   EXCEPTION
        WHEN others THEN
            v_err := substr(SQLERRM,1,200);        
...
select * from oaz22.dbamf_log_jobs order by id;
...
--Hoặc kiểm tra JOB của schema khác
 ...
 SELECT * FROM user_scheduler_jobs WHERE job_name = 'ADD_DATA_FILE';

 SELECT * FROM user_scheduler_job_log WHERE job_name = 'ADD_DATA_FILE';


--Hoặc 
select * from oaz22.dbamf_log_op;

select * from oaz22.dbamf_log_jobs ;
