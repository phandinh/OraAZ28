-- Tao bang partition theo ngay
CREATE TABLE oaz10.table_daily
...
-- Add bo sung partition theo ngay nam 2020, 2021
DECLARE
   v_nam          NUMBER (4) := 2020;
   v_owner        varchar2 (50) := 'oaz10';
   v_tablename    VARCHAR2 (50) := 'table_daily';
...
-- Tao index
create index oaz10.table_daily_pk on oaz10.table_daily(id,start_datetime) online;
...
DECLARE
   ...
   cursor c1 is
     select a.* from DBA_PART_INDEXES a, DBA_TAB_PARTITIONS b where a.owner=B.TABLE_OWNER  and a.table_name=B.TABLE_NAME  and b.table_owner='OAZ10' and b.table_name='TABLE_DAILY'  and a.index_name not like '%$%' 
     and b.partition_name like '%2020%'  order by a.owner,a.index_name;
BEGIN
   v_numday:=v_date_to-v_date_from; 
   FOR i1 in c1
   LOOP
