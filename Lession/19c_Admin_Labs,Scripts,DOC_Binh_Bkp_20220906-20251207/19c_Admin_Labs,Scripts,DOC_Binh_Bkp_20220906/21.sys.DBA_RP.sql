   ...
   PROCEDURE hc_sms
    ...
    BEGIN
        ...
        --if (to_char(sysdate,'hh24') in ('08','17')) then
            select count(*) into v_active_session from gv$session where  status='ACTIVE' and username  NOT in ('SYS','SYSMAN','DBSNMP','GGATE','GOLDENGATE') and type != 'BACKGROUND' AND TYPE = 'USER' ;
            ...
            v_all := v_all || 'Gold_HC_' || to_char(sysdate,'dd/mm hh24:mi')||':Active ('||v_active_session || '),TAS('||v_total_active_session||'),TS('||v_total_session||'),Lock('||v_lock||')';
   ...
   END;
    ...
    BEGIN
        ...
        if (to_char(sysdate,'hh24') in ('07','08','09','10','11','14','15''16','17','18','19','20','21')) then
          select count(*) into nActiveSession from gv$session where  status='ACTIVE' and username  NOT in ('SYS','SYSMAN','DBSNMP','GGATE','GOLDENGATE') and type != 'BACKGROUND' AND TYPE = 'USER' ;
          ...
          send_sms_all('DBA_GOLD','Gold_HeathCheck: ActiveSession ('||nActiveSession || '),TotalActiveSession('||nTotalActiveSession||'),TotalSession('||nTotalSession||'),Lock('||nLock||'),' ||to_char(sysdate,'dd/mm/yyyy hh24:mi:ss'));
          --sys.send_sms_ht('DBA_GOLD','Gold_HeathCheck:' || chr(10) || 'ActiveSession: ('||nActiveSession || ');' || chr(10) ||'TotalActiveSession:('||nTotalActiveSession||');' || chr(10) ||'TotalSession:('||nTotalSession||');' || chr(10) ||'Lock:('||nLock||');' || chr(10) || 'Report time:' ||to_char(sysdate,'dd/mm/yyyy hh24:mi:ss'));
          --send_sms_binhtv('Gold_HeathCheck:' || chr(10) || 'ActiveSession: ('||nActiveSession || ');' || chr(10) ||'TotalActiveSession:('||nTotalActiveSession||');' || chr(10) ||'TotalSession:('||nTotalSession||');' || chr(10) ||'Lock:('||nLock||');' || chr(10) || 'Report time:' ||to_char(sysdate,'dd/mm/yyyy hh24:mi:ss'));
          --dbms_output.put_line('Gold_HeathCheck:' || chr(10) || 'ActiveSession: ('||nActiveSession || ');' || chr(10) ||'TotalActiveSession:('||nTotalActiveSession||');' || chr(10) ||'TotalSession:('||nTotalSession||');' || chr(10) ||'Lock:('||nLock||');' || chr(10) || 'Report time:' ||to_char(sysdate,'dd/mm/yyyy hh24:mi:ss'));
      end if;
         insert into binhtv.dbamf_log_jobs (id,name,status,event_date, note)
